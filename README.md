...
- **Clerk proxy URL auto-detection (critical session fix)** — The frontend Clerk JS was not using the backend proxy in production because `VITE_CLERK_PROXY_URL` was never set at build time. Without the proxy, session cookies were scoped to `frontend-api.clerk.dev` (a different domain); browsers blocked them on every request, causing the session to drop — blank `/app` → redirected back to sign-in. Fixed by deriving the proxy URL from `window.location.origin` at runtime on production hostnames (anything that isn't `*.replit.dev` or `localhost`), with no new environment variables required.
- **AppPage loading screen** — Replaced Clerk's `<Show when="signed-in">` wrapper with explicit `useAuth` state: shows a spinner while Clerk initialises instead of a blank white page, then redirects to `/` only once it has definitively confirmed the user is signed out.
