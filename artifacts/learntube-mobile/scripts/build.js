...
// Use a dedicated Metro port for production builds so it never conflicts with
// the dev workflow (which Metro also starts on 8081 by default).
const METRO_PORT = Number(process.env.METRO_BUILD_PORT || '19001');
...
async function startMetro(expoPublicDomain, expoPublicReplId) {
  ...
  const apiBaseUrl = `https://${expoPublicDomain}`;

  console.log('Starting Metro...');
  console.log(`Setting EXPO_PUBLIC_DOMAIN=${expoPublicDomain}`);
  console.log(`Setting EXPO_PUBLIC_API_BASE_URL=${apiBaseUrl}`);
  console.log(`Setting EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY=${clerkPublishableKey.slice(0, 10)}...`);
  const env = {
    ...process.env,
